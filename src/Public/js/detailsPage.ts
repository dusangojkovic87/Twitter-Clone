...
function partialNavigationListener() {
  let tweetMenuNavList = document.querySelectorAll(
    ".tweet__menu__nav"
  ) as NodeList;
  tweetMenuNavList.forEach((tweetNav: Node) => {
    let nav = tweetNav as HTMLAnchorElement;
...
function followUserListener() {
  let folowBtn = document.querySelector(".follow__btn") as HTMLButtonElement;
  if (folowBtn) {
    folowBtn.addEventListener("click", (e: Event) => {
      let userId = getUserIdFromUrl();
      if (userId && folowBtn) {
...
function followUser(userId: string,followBtn:HTMLButtonElement) {
